...
public final class IronSpellsHelper {
    ...
    private static Method getCastingSpellIdMethod = null;
    private static boolean spellIdResolved = false;

    // getCastingSpellId stays set for the whole cast (including right-click casts), unlike the 1-tick isCasting
    public static boolean isCastingAnySpell() {
        if (!isLoaded()) return false;
        if (!spellIdResolved) {
            spellIdResolved = true;
            try {
                Class<?> cmd = Class.forName("io.redspace.ironsspellbooks.player.ClientMagicData");
                getCastingSpellIdMethod = cmd.getMethod("getCastingSpellId");
            } catch (Throwable ignored) {}
        }
        if (getCastingSpellIdMethod == null) return false;
        try {
            Object v = getCastingSpellIdMethod.invoke(null);
            if (v instanceof String s) return !s.isEmpty() && !"none".equals(s) && !"irons_spellbooks:none".equals(s);
        } catch (Throwable ignored) {}
        return false;
    }
    ...
    // Instant casts (Firebolt) finish within a tick, so latch the key to hold the aim past the projectile spawn
